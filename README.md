...
**This pool is being further developed to provide an easy to use pool for FUNCOIN miners. Testing and bug submissions are welcome!**
...
## Guide to make your very own FUNCOIN mining pool
...
### Install multi-geth
    ...
    $ git clone https://github.com/funcoinio/go-funcoinhttps://github.com/funcoinio/go-funcoin
    $ make
...
If you use Ubuntu, it is easier to control services by using serviced.
    ...
    $ sudo nano /etc/systemd/system/funcoin.service

Copy the following example

```
[Unit]
Description=funcoin for Pool
After=network-online.target

[Service]
ExecStart=/usr/local/bin/geth --funcoin --cache=1024 --rpc --extradata "Mined by <your-pool-domain>" --ethstats "<your-pool-domain>:funcoin@clostats.net"
User=<your-user-name>
...
Then run multi-geth by the following commands
    ...
    $ sudo systemctl enable funcoin
    $ sudo systemctl start funcoin

If you want to debug the node command

    $ sudo systemctl status funcoin
...
### Install funcoin Pool

    $ git clone https://github.com/funcoinio/funcoin-pool
    $ cd open-funcoin-pool
    $ make all

If you face open-funcoin-pool after ls ~/open-funcoin-pool/build/bin/, the installation has completed.

    $ ls ~/open-funcoin-pool/build/bin/
...
After=funcoin.target

[Service]
Type=simple
ExecStart=/home/<your-user-name>/open-funcoin-pool/build/bin/open-funcoin-pool /home/<your-user-name>/open-funcoin-pool/config.json
...
### Modify configuration file
    ...
    $ nano ~/open-funcoin-pool/www/config/environment.js

Make some modifications in these settings.

    BrowserTitle: 'funcoin Mining Pool',
...
The frontend is a single-page Ember.js application that polls the pool API to render miner stats.
    ...
    $ bower install
    $ ./build.sh
    $ cp -R ~/open-funcoin-pool/www/dist ~/www
